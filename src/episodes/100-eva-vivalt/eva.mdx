...
In this episode, we discuss Eva's work on causal inference and forecasting in the social sciences. We begin by discussing how surprisingly few studies in development economics tend to generalise across different contexts and the challenges this creates for global health interventions. We touch upon the differences between "internal" and "external" validity, how academic studies affects beliefs and potential consequences for longtermism.

The second part of our discussion is about forecasting and Eva's involvement in setting up the Social Science Prediction Platform. This site tests the ability of researchers to guess the results of studies in advance accurately. Eva talks about how this opens up a fascinating opportunity to spot the most novel studies and how this might help combat the bias against null results in academia.
...
![Studies and results](images/studies.png)
...
For example, suppose you run a deworming programme in a rural Kenyan village. You [find](https://cega.berkeley.edu/assets/cega_research_projects/1/Identifying-Impacts-on-Education-and-Health-in-the-Presence-of-Treatment-Externalities.pdf) that this treatment ($T$) has outstanding effects on people's health ($Y$). Does this mean that running the same treatment in a UK village will produce similar benefits? Clearly not — hookworms aren't a problem in the UK, so deworming won't have an effect. Note that by the same logic running the same treatment in the *same* Kenyan village again will likely produce lower results, as we have eliminated most of the hookworms the first time.

These examples are perhaps oversimplified, but we see an important point. What matters for the effectiveness of a deworming programme are the baseline rates. If there are not many worms, to begin with, then deworming will not have much of an effect. So will running the same treatment in a *different* Kenyan village produce similar benefits? It's not apparent that it will! It could be that the baseline worm infection rate is very different.
...
We run this experiment in two contexts, one with additional context $C$ and one without. This context is somehow linked to our treatment $T$ (e.g. the baseline hookworm rate matters for deworming effectiveness but differs in two villages). So our "true" equation is actually:
...
Going back to our example of deworming in rural Kenyan villages, we can now use this framework better understand the infamous '[worm wars](https://blogs.worldbank.org/impactevaluations/worm-wars-anthology)'. This academic debate was essentially about whether the original findings by [Miguel & Kremer (2004)](https://cega.berkeley.edu/assets/cega_research_projects/1/Identifying-Impacts-on-Education-and-Health-in-the-Presence-of-Treatment-Externalities.pdf) — which showed that deworming was one of the most cost-effective interventions to boost education and health outcomes — are accurate and generalisable across different contexts.

The debate mattered a great deal because MK's findings are widely used as the benchmark for deworming interventions. GiveWell directly cites MK as why they have deworming charities as four of their [top recommendations](https://www.givewell.org/charities/top-charities).

There are many interesting statistical rabbit-holes that the "worm wars" delve into, and really this whole saga deserves its own episode (other podcasts have already done this, like [here](https://play.acast.com/s/rationallyspeaking/rationally-speaking-180---david-roodman-on-the-worm-wars) and [here](https://www.globaldispatchespodcast.com/the-worm-wars/)). However, one aspect particularly worth highlighting for our context is the [El Niño](https://en.wikipedia.org/wiki/El_Niño) aspect.

When MK did their RCT in Kenya, they ran the experiment during a time of extraordinary flooding caused by the El Niño climate pattern (which happens about once every five years). This fact meant that villages in the study had abnormally high infection rates, [possibly](https://blog.givewell.org/2012/12/05/revisiting-the-case-for-developmental-effects-of-deworming/#ExtValidity) up two-three times the standard rate. Hence, there is a reason to believe that deworming would have been unusually effective during the study and not generalise to the broader contexts that GiveWell cares about.

Essentially, we can think of El Niño as an "additional context" $C$, which creates a heterogeneous treatment effect. MK ended up measuring $\delta_3+\delta_1$, when most NGOs and governments are interested in $\delta_1$. Relying on El Nino estimates could overstate the actual treatment effect by $\delta_3$, which is potentially enough of an upwards bias that deworming isn't one of the most cost-effective interventions out there.

One way to solve this problem is to run a new set of RCTs altogether and try just to measure $\delta_1$. But doing this at the large scale that MK did is very expensive, and long-term results are still coming out [20 years later](https://www.nber.org/papers/w27611). An alternative way is to be careful when interpreting these results in other contexts. For example, GiveWell still uses MK's studies but [discount](https://docs.google.com/spreadsheets/d/16XOOB1oWse1ICbF0OVXUYtwWwpvG3mxAAQ6LYAAndQU/edit#gid=472531943) their findings by 13% and [explicitly adjust](https://docs.google.com/document/d/18G1ibple0cSP5qk9dCyru_MLe6ghc8qbIqC1ryJCi3o/edit#) for countries' different worm burdens. Thereby GiveWell essentially tries to estimate what $\delta_3$ is and tease out $\delta_1$ accordingly.

![Aid Grade](images/aidgrade.jpg)
...
(Eva also mentions how an alternative reason might be that academics and NGOs are also more incentivised to get significant results, so there might be more p-hacking or specification searching at play. NGOs also typically run studies with smaller samples, which are generally more affected by outliers that drive up effects)
...
Eva also briefly reflects on how sometimes these two concepts are at odds. In her study on basic income, she talks about some of the design issues she had to make early on when deciding whether to include homeless people in the sample:

> This was essentially a question of internal versus external validity. In some sense, it would be more externally valid to include [homeless people]. But the costs of including them would mean that we might lose a little bit on the internal validity side because, with the additional costs, we would have a smaller sample and get less precise estimates. So there are actually some real-world trade-offs between the two that people often don't realise.
...
## "[How Much Can We Generalise From Impact Evaluations?](https://academic.oup.com/jeea/article-abstract/18/6/3045/5908781?redirectedFrom=fulltext)"
...
A Type S error essentially looks to see if, based on other studies, our inference about different study's results will have the correct sign. For example, if all the papers suggest that deworming pills increase children's height, then a Type S error will occur if a new paper finds it to have a negative effect.
...
- Eva's [website](https://evavivalt.com/)
- Aid Grade's [website](https://www.aidgrade.org/)
